{ lib, stdenv, vim, vimPlugins, buildEnv, writeText, writeTextFile
  ...
  callNixCats = nixCats:
    { allPlugins, opt, python3link, packageName, allPython3Dependencies, ... }:
  let
    ...
    start = builtins.listToAttrs
      (builtins.filter (entry: ! (grammarMatcher entry)) allPluginsMapped);

    fullDeps = {
      allPlugins = {
        inherit start treesitter_grammars;
  ...
  nixCatsDir = nixCats: writeTextFile {
    name = "nixCats-special-rtp-entry-nixCats";
    text = ''
  ...
  let
    ...
    let
      ...
      resolvedCats = callNixCats nixCats
        { inherit allPlugins opt python3link packageName allPython3Dependencies; };

      packdirStart = vimFarm "pack/${packageName}/start" "packdir-start"
            (allPlugins ++ [ resolvedCats (nixCatsDir resolvedCats) ]);
